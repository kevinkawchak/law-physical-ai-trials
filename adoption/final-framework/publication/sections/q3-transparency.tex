...
transparency to users and traceability of decisions at the center of trustworthiness \cite{fda2021samd}.
...
specifies exactly which elements are captured at each of the steps and how they are all bound
...
the test the FDA direction implies.

\begin{cfwfig}
\node[cftwo] (a) at (0,0) {Action}; \node[cfact] (b) at (4.2,0) {Hash-chained record};
\node[cfhope] (c) at (8.4,0) {Auditor reconstructs};
\draw[cfedge] (a)--(b); \draw[cfedge] (b)--(c);
\draw[cfedged] (b) to[out=120,in=60, looseness=0.7] (a.north);
\end{cfwfig}
\vspace{-0.75cm}
\cfwcaption{Figure 5. The audit trail: each
...
\footnotesize
\begin{tabularx}{\textwidth}{@{}L{2.5cm} Y L{3.2cm}@{}}
\toprule
...
Countersignature & The named human who accepted, escalated, or overrode the action & Tumor board, IRB\\
