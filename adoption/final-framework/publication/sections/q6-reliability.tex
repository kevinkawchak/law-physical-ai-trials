...
\begin{cfwfig}
\node[cfone] (a) at (0,0) {Estimate}; \node[cftwo] (b) at (4,0) {Confidence interval}; \node[cfdec] (c) at (7.6,0) {Within bound?};
\node[cfhope] (d) at (11,0.8) {Report}; \node[cfthree] (e) at (11,-0.8) {Escalate};
\draw[cfedge] (a)--(b); \draw[cfedge] (b)--(c); \draw[cfedge] (c)--(d); \draw[cfedge] (c)--(e);
\end{cfwfig}
\vspace{-0.75cm}
\cfwcaption{Figure 8. The uncertainty gate :
...
\footnotesize
\begin{tabularx}{\textwidth}{@{}L{3.2cm} Y L{5.5cm}@{}}
\toprule
\textbf{Control} & \textbf{Trigger} & \textbf{Response}\\
\midrule
Reproducibility check & Same input yields a different output than recorded & Halt and investigate before further use\\
Drift monitor & Input distribution or perf. shifts beyond set threshold & Flag the drift, open continuous monitoring\\
Uncertainty bound & Estimate's confidence interval exceeds gate tolerance & Escalate the case to a qualified human\\
Scheduled revalidation & Fixed cadence or a triggering drift event is reached & Re-run the credibility assessment and gates\\
\bottomrule
\end{tabularx}
