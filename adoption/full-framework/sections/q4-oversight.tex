...
path that routes the decision to a qualified human instead of acting alone.
...
escalation the human is summoned, and on override the human simply stops the action.
...
\end{cfwfig}
\vspace{-0.2cm}
\cfwcaption{Figure 6. The oversight loop (Mermaid 09 reproduced as colored TikZ): a
...
Human-on-the-loop & Above a running process within pre-authorized bounds & Monitor in real time and intervene at will\\
Escalation & Summoned when a proposal exceeds the gates' bounds & Decide the escalated case as a qualified human\\
Override and stop & Outside the gates, at any moment & Halt the system unconditionally, no gate required\\
\bottomrule
\end{tabularx}
