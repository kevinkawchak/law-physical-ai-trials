...
\node[psgtwo] (r) at (4.25,0) {Independent\\review};
\node[psgdec] (g) at (8.25,0) {Ten\\gates};
\node[psgevid] (a) at (12.2,1.2) {ACCEPT\\under audit};
\node[psgthree] (e) at (12.2,0) {ESCALATE\\to a human};
\node[psgcaut] (b) at (12.2,-1.2) {BLOCK\\before patient};
\draw[psgedge] (p)--(r); \draw[psgedge] (r)--(g);
\draw[psgedge] (g)--(a.south west); \draw[psgedge] (g)--(e.west);
\draw[psgedge] (g)--(b.north west);
\draw[psgedged] (e) to[out=180,in=-10,looseness=1] (g.south east);
\end{psgfig}
\vspace{0.15cm}
\psgcaption{\textbf{Figure 4.} Verification before generation: every action
is gated to ACCEPT, ESCALATE, or BLOCK before the patient.}
\label{fig:vbg}
...
\footnotesize
\begin{tabularx}{\textwidth}{@{}L{0.5cm} L{4.75cm} C{0.7cm} C{2.75cm} Y@{}}
\toprule
...
standards. The three hard catastrophe predicates must all hold at 1.00.}
\label{tab:gates}
\end{table}
