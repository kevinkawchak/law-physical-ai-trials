...
\autoref{tab:mandate} states each gap and what it leaves unaddressed today.

\begin{psgfig}
\node[psgone] (e) at (0,0) {Embodiment\\problem};
\node[psgtwo] (s) at (4.0,0) {Sequencing\\gap};
\node[psgthree] (f) at (8.0,0) {Financial-data\\gap};
\node[psgact] (n) at (4.0,-2.1) {The mandate\\for a statute};
\draw[psgedge] (e) to[out=-90,in=160,looseness=0.7] (n.north west);
\draw[psgedge] (s)--(n.north);
\draw[psgedge] (f) to[out=-90,in=20,looseness=0.7] (n.north east);
\end{psgfig}
...
constituent who lost someone to a preventable error.
